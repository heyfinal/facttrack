...
#   party-splitter → publicsearch_docs fetch (multi-page) → OCR every page
#   → clause extraction → curative engine → report rendering
#
...
echo "[1/6] party split (lessor/lessee → lease_party rows + deceased flags)"
$PY -m facttrack.ingest.party_splitter --county 48001

echo "[2/6] fetch doc images (multi-page MPV)"
$PY -m facttrack.ingest.publicsearch_docs --county 48001 --max 30

echo "[3/6] OCR every cached image"
$PY -c "
...
echo "[4/6] extract clauses → lease.* columns + lease_party.is_deceased"
$PY -m facttrack.engine.clause_parser --county 48001

echo "[5/6] run curative engine"
$PY -m facttrack.engine.run --project county_research_48001

echo "[6/6] build report artifacts (map, xlsx, html, pdf)"
